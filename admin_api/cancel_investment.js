...
Router.post("/", verifyToken, async (req, res) => {
  try {
    const request_isvalid = validate_admin_cancel_investment(req.body);
    if (!request_isvalid) {
      return res.status(400).json({ error: true, errMessage: "Invalid input" });
    }
    ...
    const investment = await Investment.findById(req.body.investment).populate(
      "user",
    );
    if (!investment) {
      return res.status(400).json({
        error: true,
        errMessage:
          "The investment you requested to cancel no longer exists. Please refresh and try again.",
      });
    }
    ...
    await Promise.all([
      user.save(),
      Investment.findByIdAndDelete(req.body.investment),
    ]);
    ...
    return res
      .status(500)
      .json({ error: true, errMessage: "Internal server error" });
  }
});
